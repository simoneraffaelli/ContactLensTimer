...
public class PullToDismiss extends FrameLayout {
    private PullToDismiss.Listener listener;
    private ViewDragHelper dragHelper;
    private float minFlingVelocity;
    private float verticalTouchSlop;
    private boolean animateAlpha;

    public PullToDismiss(@NonNull Context context) {
        super(context);
        init(context);
    }

    public PullToDismiss(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public PullToDismiss(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    @TargetApi(21)
    public PullToDismiss(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle, int defResStyle) {
    ...
    private void init(@NonNull Context context) {
        if (!isInEditMode()) {
            ...
            dragHelper = ViewDragHelper.create(this, new PullToDismiss.ViewDragCallback(this));
        }
    }
    ...
    private static class ViewDragCallback extends Callback {
        private final PullToDismiss pullDismissLayout;
        private int startTop;
        private float dragPercent;
        private View capturedView;
        private boolean dismissed;

        private ViewDragCallback(PullToDismiss layout) {
        ...
        public void onViewReleased(@NonNull View view, float xv, float yv) {
            ...
            Log.d("VEL", "" + pullDismissLayout.minFlingVelocity);
            Log.d("VEL", "" + pullDismissLayout.dragHelper.getMinVelocity());
            pullDismissLayout.invalidate();
        }
    }
}
